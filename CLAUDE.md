...
VideoForge is a **client-side-first** video toolkit (Next.js 16 App Router, React 19, MUI 9). All media processing runs in the browser via WebAssembly and nothing is uploaded — the **one** exception is optional Hugging Face processing, active only when the user supplies an HF token: audio slices for hosted transcription and the finished transcript text for AI polish (see below). It ships as three runtimes from one codebase: a Next.js standalone server, a static-export desktop web bundle, and a Tauri desktop app.
...
- **Transcription pipeline**: FFmpeg normalizes to 16 kHz WAV (0–15% of progress bar) → `decodeWavToPcm16k` ([audioUtils.ts](src/utils/audioUtils.ts)) → Whisper runs in a **Web Worker** ([whisper.worker.ts](src/workers/whisper.worker.ts), model download 15–80%, inference 80–100%) → transcript serialized to txt/srt/vtt ([subtitleUtils.ts](src/utils/subtitleUtils.ts)). `useTranscriber` bridges the worker with request-id correlation and progress callbacks. Both paths stream partial text per 60s segment via `onPartialText`.
- **Optional hosted transcription** ([hfTranscribe.ts](src/utils/hfTranscribe.ts)): when a user-supplied HF token ([hfToken.ts](src/utils/hfToken.ts), localStorage) is set, the browser is online, and `translate` is off, `useFileConverter` POSTs 60s audio slices to HF's hosted `whisper-large-v3` instead of the local worker. **This is the one path that uploads audio off-device** — token stays in localStorage, sent browser→HF directly (no server proxy, so it works in all three runtimes). Any hosted failure (offline/CORS/HTTP/parse) falls back to the local worker via try/catch and surfaces a non-blocking warning notice, so the offline guarantee holds whenever no token is set.
- **Optional AI transcript polish** ([hfPolish.ts](src/utils/hfPolish.ts)): with a token set (any transcription engine), the finished plain text is sent browser→HF chat completions in ≤3500-char sentence blocks for correction, over a **model fallback ladder** (`POLISH_MODELS`: `swiss-ai/Apertus-8B-Instruct-2509:publicai` → `speakleash/Bielik-11B-v3.0-Instruct:publicai`, pinned once one answers). Apertus is first because Bielik requires accepting an additional model-access condition; the explicit `publicai` provider selection targets Public AI's currently free public-access utility instead of consuming the paid-provider route; pricing and availability may change. 402/403 advance to the next configured candidate (403 can mean the selected model is gated), while 401/429 abort it — **the second token-gated upload path (transcript text)**. The ladder exists because HF routes to third parties and a model can be unavailable to an account (provider down); the router can answer 503, and **its 503 path omits CORS headers**, so the browser reports an opaque `TypeError: Failed to fetch` with no status. Treat a status-less polish failure as "try the next model", not "offline". Per-block sanity guardrails (length ratio, `finish_reason`) keep raw text on suspicious output; any failure keeps the raw transcript + shows a notice. Polish touches only the panel/`txt` output — `srt`/`vtt` chunks stay raw.

### Whisper worker specifics

Uses `@huggingface/transformers` with model `onnx-community/whisper-small`. Has its own **device fallback ladder**: WebGPU (q4) → WASM q8 with graph optimization *disabled* → WASM fp32. The disabled-optimization and same-origin ONNX path (`/ort/`, populated by `prepare-onnx-assets.mjs`) both work around bugs/CORS in the pinned dev-prerelease onnxruntime-web build — read the long comments in the worker before touching device/dtype config. Transcription runs in fixed 60-second windows for real progress reporting and to curb Whisper repetition.
