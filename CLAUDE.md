...
- **Optional AI transcript polish** ([hfPolish.ts](src/utils/hfPolish.ts)): with a token set (any transcription engine), the finished plain text is sent browser→HF chat completions (`Qwen/Qwen2.5-7B-Instruct`) in ≤3500-char sentence blocks for correction — **the second token-gated upload path (transcript text)**. Per-block sanity guardrails (length ratio, `finish_reason`) keep raw text on suspicious output; any failure keeps the raw transcript + shows a notice. Polish touches only the panel/`txt` output — `srt`/`vtt` chunks stay raw.
