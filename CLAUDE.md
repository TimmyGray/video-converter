...
- **Optional AI transcript polish** ([hfPolish.ts](src/utils/hfPolish.ts)): with a token set (any transcription engine), the finished plain text is sent browser→HF chat completions in ≤3500-char sentence blocks for correction, over a **model fallback ladder** (`POLISH_MODELS`: Qwen3-4B-Instruct → Llama-3.1-8B-Instruct → gemma-3-4b-it → `FREE_TIER_POLISH_MODEL`, cheapest-capable first, one provider each, pinned once one answers). HF's free tier is $0.10/month of credits (PRO $2.00) and an hour of transcript costs ~$0.0005 at the head of the ladder (~200 h/month free), so the ladder ends in a zero-cost model to survive credit exhaustion — hence 402 advances the ladder while 401/403/429 abort it — **the second token-gated upload path (transcript text)**. The ladder exists because HF routes to third-party providers and a model can be unavailable to an account (no credits, provider down) — the router answers 503, and **its 503 path omits CORS headers**, so the browser reports an opaque `TypeError: Failed to fetch` with no status. Treat a status-less polish failure as "try the next model", not "offline". Per-block sanity guardrails (length ratio, `finish_reason`) keep raw text on suspicious output; any failure keeps the raw transcript + shows a notice. Polish touches only the panel/`txt` output — `srt`/`vtt` chunks stay raw.
