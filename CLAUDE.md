...
VideoForge is a **client-side-first** video toolkit (Next.js 16 App Router, React 19, MUI 9). All media processing runs in the browser via WebAssembly and nothing is uploaded — the **one** exception is optional hosted transcription, which uploads audio to Hugging Face only when the user supplies an HF token (see below). It ships as three runtimes from one codebase: a Next.js standalone server, a static-export desktop web bundle, and a Tauri desktop app.
